...
def split_domain(net: LiRPANet, domains, d, batch, stats=None,
    ...
    for decision in branching_decision:
        if decision is None:
            continue
        layer_idx = int(decision[0])
        if layer_idx < 0 or layer_idx >= len(net.split_nodes):
            continue
        layer_name = net.split_nodes[layer_idx].name
        layer_hist[layer_name] = int(layer_hist.get(layer_name, 0)) + 1
