...
class ABCROWN:
    ...
    def main(self, interm_bounds=None):
        ...
        base_branching_candidates = bab_args['branching']['candidates']
        ...
        if cache_cfg['enabled'] and cache_cfg['strict_recompute']:
            # Safety first for cached warm starts: avoid directly trusting refined bounds.
            arguments.Config['solver']['skip_with_refined_bound'] = False
        ...
        for new_idx, csv_item in enumerate(example_idx_list):
            ...
            arguments.Globals['branching_layer_hist'][new_idx] = {}
            ...
            if cache_cfg['enabled']:
                ...
                if cache_cfg['branching_hints']:
                    ...
                    if hint.get('candidates_delta', 0) > 0:
                        old_c = bab_args['branching']['candidates']
                        bab_args['branching']['candidates'] = min(old_c + hint['candidates_delta'], 8)
                        print(
            ...
            if (not verified_success and enable_incomplete):
                ...
                if (
                    cache_cfg['enabled']
                    and self._cache_model_sig is not None
                    and self._cache_spec_sig is not None
                    and isinstance(ret, dict)
                    and ret.get('alphas', None) is not None
                ):
                    self.cache_manager.save_for_instance(
                        self._cache_model_sig,
            ...
            if (
                ...
                if len(self.logger.bab_ret) > 0 and self.logger.bab_ret[-1][0] == new_idx:
                    domains_visited = self.logger.bab_ret[-1][2]
                instance_time = self.logger.status_per_sample_list[-1][1]
                layer_hist = arguments.Globals.get('branching_layer_hist', {}).get(new_idx, {})
                self.cache_manager.save_for_instance(
                    self._cache_model_sig,
                    self._cache_spec_sig,
                    alpha=None,
                    instance_time_sec=instance_time,
